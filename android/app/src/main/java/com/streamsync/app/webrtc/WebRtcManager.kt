...
import com.streamsync.app.audio.HornSoundPlayer
import com.streamsync.app.audio.SystemAudioMixer
import com.streamsync.app.audio.VoiceNoiseSuppressor
...
class WebRtcManager(
    ...
    // Participant events (for notifications) - SharedFlow so rapid events don't drop
    private val _participantEvent = MutableSharedFlow<ParticipantEvent>(
        replay = 0,
        extraBufferCapacity = 16,
        onBufferOverflow = BufferOverflow.DROP_OLDEST
    )
    val participantEvent: SharedFlow<ParticipantEvent> = _participantEvent.asSharedFlow()
    ...
    private fun handleParticipantChanges(participants: List<FirebaseSignaling.Participant>) {
        ...
        newJoins.forEach { participant ->
            _participantEvent.tryEmit(ParticipantEvent.Joined(participant.userId))
            Log.d(TAG, "Participant joined: ${participant.userId}")
        }

        newLeaves.forEach { participant ->
            _participantEvent.tryEmit(ParticipantEvent.Left(participant.userId))
    ...
    private fun handleRemoteScreenShareStatus(isSharing: Boolean, sharerId: String) {
        ...
        if (sharerId != myId) {
            ...
            if (isSharing) {
                ...
                _participantEvent.tryEmit(ParticipantEvent.StartedSharing(sharerId))
            } else {
                Log.d(TAG, "Emitting StoppedSharing event for $sharerId")
                _participantEvent.tryEmit(ParticipantEvent.StoppedSharing(sharerId))
            }
        } else {
    ...
    private fun createOffer(roomId: String) {
        ...
        peerConnection?.createOffer(object : SdpObserver {
            override fun onCreateSuccess(sdp: SessionDescription?) {
                sdp?.let { raw ->
                    val munged = preferH264(raw)
                    Log.d(TAG, "Offer created")
                    peerConnection?.setLocalDescription(SimpleSdpObserver("setLocalDescription (offer)"), munged)
                    scope.launch {
                        signaling.sendOffer(roomId, munged)
                    }
                }
            }
        ...
        peerConnection?.createAnswer(object : SdpObserver {
            override fun onCreateSuccess(sdp: SessionDescription?) {
                sdp?.let { raw ->
                    ...
                    peerConnection?.setLocalDescription(SimpleSdpObserver("setLocalDescription (answer)"), munged)
                    scope.launch {
                        signaling.sendAnswer(roomId, munged)
                    }
                }
            }
    ...
    /**
     * Reorder the m=video line so H264 payload types come first.
     * Encourages the remote to pick our hardware H264 encoder/decoder path,
     * which materially reduces glass-to-glass latency vs. software VP8.
     */
    private fun preferH264(sdp: SessionDescription): SessionDescription {
        val lines = sdp.description.split("\r\n").toMutableList()
        val mLineIdx = lines.indexOfFirst { it.startsWith("m=video ") }
        if (mLineIdx < 0) return sdp

        val h264Pts = mutableListOf<String>()
        for (line in lines) {
            if (line.startsWith("a=rtpmap:")) {
                val parts = line.removePrefix("a=rtpmap:").split(" ")
                if (parts.size == 2 && parts[1].startsWith("H264", ignoreCase = true)) {
                    h264Pts.add(parts[0])
                }
            }
        }
        ...
        val mLine = lines[mLineIdx].split(" ").toMutableList()
        if (mLine.size < 4) return sdp
        val header = mLine.subList(0, 3) // "m=video", port, proto
        val pts = mLine.subList(3, mLine.size).toMutableList()
        pts.removeAll(h264Pts)
        val reordered = header + h264Pts + pts
        lines[mLineIdx] = reordered.joinToString(" ")

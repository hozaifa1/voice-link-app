...
- [ ] **Step 1.1 — [Human] Verify Step 1 scaffolding works end-to-end**
  - **Open + Sync**
    - Open Android Studio → `File > Open` → select: `f:\voiceapp\android`
    - Confirm Gradle sync completes successfully (no red errors in the Build tool window)
    - Confirm Android Studio is using a JDK that works:
      - `File > Settings > Build, Execution, Deployment > Build Tools > Gradle`
      - Set `Gradle JDK` to `Embedded JDK (17)` (recommended)
    - Confirm Gradle is set to use the wrapper:
      - Same screen: `Build and run using` = `Gradle`
      - Same screen: `Run tests using` = `Gradle`
  - **Confirm SDK + package invariants**
    - Open `android/app/build.gradle.kts` and verify:
  ...
  - **Confirm key dependencies resolve**
    - Open `android/gradle/libs.versions.toml` and confirm versions are present for:
      - Android Gradle Plugin (AGP)
      - Kotlin
      - OkHttp
      - Coroutines
      - WebRTC
    - Sync again and confirm there are no “Failed to resolve …” errors for:
      - WebRTC
      - OkHttp
      - Kotlin Coroutines
  ...
  - **Build checks**
    ...
    - Optional but recommended: verify both build variants compile:
      - Open `Gradle` tool window → `:app` → `Tasks` → `build`
      - Run `assembleDebug`
      - Run `assembleRelease`
  - **Unit test**
    - Run: `ExampleUnitTest` (from `android/app/src/test/...`)
    - Confirm it passes
  - **Run on device/emulator**
    - Select `app` run configuration
    - Run on a device/emulator with Android 10+ (API 29+)
    - Confirm the Home screen renders with:
      - App title: `VoiceLink Connect`
      - Room code input
      - `Join Room` button (disabled until 6 chars entered)
      - `Create New Room` button
    - Confirm the room code input behavior:
      - Typing letters becomes uppercase
      - Input is capped at 6 characters
      - `Join Room` becomes enabled exactly at 6 characters
    - Confirm the app does not crash on first launch (check Logcat for exceptions)
  - **Manifest sanity**
    - Open `android/app/src/main/AndroidManifest.xml`
    - Confirm the Application class is declared:
      - `android:name=".VoiceLinkApp"`
    - Confirm permissions exist:
      - `RECORD_AUDIO`, `INTERNET`, `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_MEDIA_PROJECTION`
    - Confirm service is declared:
      - `.service.AudioCaptureService` with `android:foregroundServiceType="mediaProjection"`
  - **Config sanity (wrapper timeout)**
    - Open `android/gradle/wrapper/gradle-wrapper.properties`
    - Confirm `networkTimeout=600000` exists
  - **File layout sanity (quick spot-check)**
    - Confirm these files exist:
      - `android/app/src/main/java/com/voicelink/connect/MainActivity.kt`
      - `android/app/src/main/java/com/voicelink/connect/VoiceLinkApp.kt`
      - `android/app/src/main/java/com/voicelink/connect/ui/screens/HomeScreen.kt`
      - `android/app/src/main/java/com/voicelink/connect/service/AudioCaptureService.kt`
      - `android/app/src/main/java/com/voicelink/connect/webrtc/WebRtcConfig.kt`

- [ ] **Step 2 — [Human] Create Firebase project (free) + register Android app** (Appendix E; free account context in Appendix C; pacing in Appendix B)
  - Firebase Console: https://console.firebase.google.com/
  - Download `google-services.json`

- [ ] **Step 3 — [Coding] Integrate Firebase + Crashlytics (early)** (Appendix E; pacing in Appendix B)
